...
namespace LEARNER
{
  ...
  void dispatch(Ex& ec, Ctx& context, vw& master)
  {
    process<Ex, dispatch_impl>(ec, context);                   // call learn or predict
    VW::finish_example(master, ec);  // clean up
  }
  ...
  template <class Ctx>
  void dispatch_single_line(example*& ec, Ctx& context, vw& master)
  {
    if (ec->indices.size() > 1)  // 1+ nonconstant feature. (most common case first)
      dispatch<example, Ctx, learn_ex>(*ec, context, master);
    else if (ec->end_pass)
      dispatch<example, Ctx, end_pass>(*ec, context, master);
    else if (is_save_cmd(ec))
      dispatch<example, Ctx, save>(*ec, context, master);
    else
      dispatch<example, Ctx, learn_ex>(*ec, context, master);
  }
...
void generic_driver(vw& all)
{
  ...
  template <class Ex, class Ctx, bool (*produce)(vw&, Ex&, Ctx&), void (*consume)(Ex&, Ctx&, vw&)>
  void new_driver(vw& master, Ctx& context)
  {
    Ex ex;

    while (produce(master, ex, context))
      consume(ex, context, master);
